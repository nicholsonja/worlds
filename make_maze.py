...
INPUT_TEMPLATE_FILE = "maze-template.xml"
OUTPUT_WORLD_FILE = "maze_world.xml"
...
INCLUDE_TEMPLATE = """
    <model name='MODEL-NAME'>
      <static>1</static>
      <include>
        <uri>model:///home/user/worlds/small_box</uri>
	  </include>
    </model>
"""
...
if __name__ == "__main__":
    if len(argv) == 1:
        ...
        rows = int(argv[1])
        columns = int(argv[2])
        cell_width = int(argv[3])
    else:
        print(f"USAGE: {argv[0]} ROWS COLUMNS CELL_SIZE")
        exit(0)

    if rows < 2 or columns < 2 or cell_width < 2:
